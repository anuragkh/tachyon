...
if [ -z $SUCCINCT_DATA_PATH ] ; then
	SUCCINCT_DATA_PATH=/mnt/data
fi
echo "Starting QueryServer @ `hostname` with data path $SUCCINCT_DATA_PATH"
(nohup $JAVA -cp $TACHYON_JAR -Dtachyon.home=$TACHYON_HOME -Dtachyon.logger.type="WORKER_LOGGER" -Dlog4j.configuration=file:$TACHYON_CONF_DIR/log4j.properties $TACHYON_JAVA_OPTS succinct.QueryServer $MASTER_ADDRESS $SUCCINCT_DATA_PATH > $TACHYON_LOGS_DIR/succinct.server.log@$now 2>&1) &